...
def inst_handler(data: dict):
    ...
    image(inst_handler(data) or 'Sample', text_align='left', font_family='Roboto',
...
def tg_handler(data: dict):
    ...
    return {
        'text': text,
        'image_path': image(
            background=Image.open('images/tg_preview.png'),
            text=re.sub(r'\(.*?\)', '', data.get('title', 'Sample')).strip(),
            font_family='Roboto',
            font_weight='Condensed',
            top_indent=130,
            top_indent_2=130,
        )
    }
    ...
    try:
        if message['chat']['id'] in admins:
            if message['text'].lower().startswith('/vars'):
                ...
                path, width, height = 'images/tg_preview.png', 1000, 1000
                ...
                if 'inst' in message.text.lower():
                    path, width, height = 'images/ig_preview.png', 1080, 1080
